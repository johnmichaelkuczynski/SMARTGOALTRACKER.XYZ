...
export default function Journal() {
  ...
  function addImageText(t: string) {
    setText((prev) => {
      const next = prev ? `${prev}\n${t}` : t;
      if (savedTimer.current) clearTimeout(savedTimer.current);
      savedTimer.current = setTimeout(() => commit(next), 600);
      return next;
    });
  }
    ...
    <div className="space-y-10">
      ...
      <section className="rounded-xl border border-card-border bg-card p-6">
        ...
        <div className="flex items-center justify-between gap-3 mb-2">
          <label className="block text-sm text-muted-foreground">{periodPrompt(period)}</label>
          <ImageOcrButton onText={addImageText} label="Add screenshot" />
          <DocumentTextButton onText={addImageText} label="Add document" />
        </div>
        <Textarea
          value={text}
