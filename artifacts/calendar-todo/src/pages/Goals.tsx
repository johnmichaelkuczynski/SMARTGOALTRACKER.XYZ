...
import { useStore, addTask, deleteTask } from "@/lib/storage";
...
export default function Goals() {
  ...
  return (
    <div className="space-y-12">
      ...
      <GoalSection title="Medium-term" subtitle="Months out. The next mile-marker." goals={medium} completions={completions} timeframe="medium" />
      <GoalSection title="Long-term" subtitle="Years out. Where this is all going." goals={long} completions={completions} timeframe="long" />
    </div>
  );
}
...
function GoalSection({
  ...
  return (
    <section>
      ...
      <QuickAddGoal timeframe={timeframe} label={title.toLowerCase()} hasGoals={goals.length > 0} />
    </section>
  );
}
...
function QuickAddGoal({
  ...
  const defaultDate = () =>
    format(addMonths(new Date(), timeframe === "medium" ? 3 : 12), "yyyy-MM-dd");
    ...
    <div className="rounded-lg border border-dashed border-border bg-card/40 p-4">
      {!hasGoals && (
        <p className="text-sm text-muted-foreground mb-3">
          No {label} goals yet. Name one below and set a target date.
        </p>
      )}
      <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
        <div className="flex-1 space-y-1.5">
          <Label htmlFor={`goal-${timeframe}`}>Goal</Label>
          <Input
            id={`goal-${timeframe}`}
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder={`What's the ${label} goal?`}
            onKeyDown={(e) => {
        ...
        <div className="space-y-1.5">
          <Label htmlFor={`goal-date-${timeframe}`}>Target date</Label>
          <Input
            id={`goal-date-${timeframe}`}
            type="date"
            value={date}
        ...
        <Button onClick={submit} disabled={!title.trim()} className="gap-2 shrink-0">
